...
def train_with_optuna(
            ...
            if isinstance(param_value, (list, tuple)):
                if isinstance(param_value[0], bool):
                    ...
                    params[param_name] = trial.suggest_int(
                        param_name, param_value[0], param_value[-1]
                    )
                    ...
                    raise ValueError(
                        f"Unsupported parameter type for param_value[0] ({param_value[0]})."
                    )
        ...
        for i in range(len(train_val_test_data)):
            ...
            if standardize:
                ...
                train_df = pd.DataFrame(
                    standardizer.fit_transform(fold[0].drop([target_column], axis=1)),
                    columns=fold[0].columns.difference([target_column]),
                )
                train_df.index = fold[0].index
                # print(target_column in train_df.columns)
                train_df[target_column] = fold[0][target_column]
                val_df = pd.DataFrame(
                    standardizer.fit_transform(fold[1].drop([target_column], axis=1)),
                    columns=fold[1].columns.difference([target_column]),
                )
            ...
            if model_params["model"] in ["LGBMClassifier", "LGBMRegressor"]:
                ...
                model.fit(
                    train_df.drop(columns=[target_column]), train_df[target_column]
                )
    ...
    best_model.fit(
        train_val_df.drop(columns=[target_column]), train_val_df[target_column]
    )
