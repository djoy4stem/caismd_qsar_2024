...
def set_seeds(seed:int=None):
    ...
    os.environ["PYTHONHASHSEED"] = str(seed) # controls the hash seed for hash-based operations so they are reproducible, if seed is not None
...
def avg_tanimoto_coefficient_from_fps(fingerprints):
    ...
    similarities = Chem.DataStructs.BulkTanimotoSimilarity(fingerprints[0], fingerprints)
...
def avg_tanimoto_coefficient(molecules, fingerprint_type="morgan", radius=2, bitLength=2048):
    """
    ...
    fingerprints = calculate_fingerprints(molecules=molecules, fingerprint_type=fingerprint_type
                                            , radius=radius, bitLength=bitLength)
...
def avg_tanimoto_dissimilarity(molecules, fingerprint_type="morgan", radius=2, bitLength=2048):
    """
    Computes the average Tanimoto coefficient between molecules using specified fingerprints.
