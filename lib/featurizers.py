...
from rdkit.Chem import Draw, AllChem, Crippen, QED, Descriptors, GraphDescriptors, Fragments, MolToSmiles, GetPeriodicTable
...
RDKIT_FRAGS = [[frag_name, MolToSmiles(eval(f'Fragments.{frag_name}').__defaults__[1])] for frag_name in dir(Fragments) if not re.match('^fr', frag_name) is None]
...
DF_FUNC_GRPS_MINI = pd.DataFrame(
    [ ['aldehyde', '[$([CX3H][#6]),$([CX3H2])]=[OX1]']
     ...
     , ['ether', '[OX2]([#6;!$(C([OX2])[O,S,#7,#15,F,Cl,Br,I])])[#6;!$(C([OX2])[O,S,#7,#15])]']
     , ['thioether', '[SX2]([#6;!$(C([SX2])[O,S,#7,#15,F,Cl,Br,I])])[#6;!$(C([SX2])[O,S,#7,#15])]']
     ...
     , ['sulfoniamide', '[SX4;$([H1]),$([H0][#6])](=[OX1])(=[OX1])[#7X3;$([H2]),$([H1][#6;!$(C=[O,N,S])]),$([#7]([#6;!$(C=[O,N,S])])[#6;!$(C=[O,N,S])])]']
    ] ,
    columns = ['name','SMARTS']
)
...
class MoleculeFeaturizer(object):
    def __init__(self, features:List[str]=None, df_func_gps:pd.DataFrame=DF_FUNC_GRPS, label_col:str='name'):
        self.dim = 0
        self.allowable_calc_features = [
            ...
            , 'CalcNumLipinskiHBA', 'CalcNumLipinskiHBD', 'CalcNumHBA', 'CalcNumHBD', 'CalcNumSpiroAtoms', 
            'CalcNumRotatableBonds', 'CalcNumLipinskiHBD'
        ...
        self.default_mordred_descs  = ['ExtendedTopochemicalAtom', 'Polarizability'
                                        ,'ZagrebIndex', 'MoeType']
        ...
        if not features is None:
            self.rdkit_features = [f for f in features if f in self.allowable_calc_features 
                                + self.allowable_crippen_features + self.allowable_qed_features 
                                + self.allowable_descriptors + self.allowable_graph_descriptors
                                # + self.mordred_descs
                            ]
            
            # print('features', features)
            self.mordred_descs  = [f for f in features if f in dir(mordred.descriptors) if re.match('^_', f) is None and f !='all']
        else:
            self.rdkit_features = self.allowable_calc_features + self.allowable_crippen_features + \
                                    self.allowable_qed_features + self.allowable_descriptors + \
    ...
    def compute_rdkit_properties(self, molecule, features:List[str]=None, label_col:str='name', as_dict:bool=True, count_unique_bits:bool=True):
        properties = {}
        ...
        if not molecule is None:
            if not (features is None or len(features) == 0):
                for prop in features:
                    try:
                        if prop in self.allowable_calc_features:
                            ...
                            properties[prop] = eval(f'GraphDescriptors.{prop}')(molecule)
                    except:
                ...
                if len(failed_com_preps) > 0:
                    print(f"Could not compute molecular properties: {'; '.join(failed_com_preps)}")

            fcgps = None
            
            
            if not self.df_func_gps is None:
                fcgps   = get_func_groups_pos_from_mol(molecule, df_func_gps=self.df_func_gps
                                            , as_dict=True, label_col=self.fgcp_label_col, countUnique=count_unique_bits)
                
                if fcgps is None and (not features is None):
                    fcgps = dict(zip(self.df_func_gps[self.fgcp_label_col].tolist(), [None] * self.df_func_gps.shape[0]))
            ...
            if not self.df_func_gps is None:
                properties = dict(properties, **dict(zip(self.df_func_gps[self.fgcp_label_col].tolist(), [None] * self.df_func_gps.shape[0])))
    ...
    def compute_mordred_props(self, molecules, mordred_props:list=None):
        ...
        try:
            ...
            if not mordred_props is None:
                ...
                clean_props = [eval(f'descriptors.{prop}') for prop in self.default_mordred_descs]
            
            # print('clean_props', clean_props)
            if not (clean_props is None or len(clean_props) == 0):
                # print("Compute")
                return mordred.Calculator(descs=clean_props, ignore_3D=False).pandas(molecules)
            else:                
                return None
    ...
    def compute_properties_for_mols(self, molecules, as_dataframe:bool=True, count_unique_bits:bool=True):
        try:
        # if True:
            ...
            t0 = time()
            rdkit_props      = mols_df['RMol'].apply(lambda mol: self.compute_rdkit_properties(molecule=mol, count_unique_bits=count_unique_bits))
            # print('rdkit_props\n', rdkit_props[0], '\n', rdkit_props.values.tolist())
            t1 = time()
            t_rdkit= t1 - t0
            print(f"RDKIT property calculation: {round(t_rdkit, 3)} seconds.")
            rdkit_props_is_all_none =   rdkit_props[0] is None and len(rdkit_props.unique())==1
            ...
            if not (rdkit_props_is_all_none or mordred_props_df is None):
                ...
                if as_dataframe:                    
                    ...
                    return pd.concat([rdkit_props_df, mordred_props_df], axis=1).to_dict(orient='records')
...
def get_func_groups_pos_from_mol(mol, df_func_gps:pd.DataFrame=DF_FUNC_GRPS, as_dict:bool=False, label_col:str='name', countUnique:bool=True):
    ...
    try:
        ...
        if not as_dict:
            ...
            return pd.Series(onehot_func_gps.tolist(), index=df_func_gps[label_col]).to_dict()
    except:
        for i, smiles in enumerate(df_func_gps['SMARTS']):
...
def get_func_groups_pos(smiles, df_func_gps:pd.DataFrame=DF_FUNC_GRPS, as_dict:bool=False, label_col:str='name'):
...
def num_halogen_atoms(molecule: Mol):
    if not molecule:
        ...
        return len([atom for atom in molecule.GetAtoms() if atom.GetSymbol() in ('Cl', 'Br', 'I', 'F')])
...
def halogen_weight(molecule: Mol):
    if not molecule:
        ...
        halogen_atoms = [atom for atom in molecule.GetAtoms() if atom.GetSymbol() in ('Cl', 'Br', 'I', 'F')]
...
def num_sp3_carbons(molecule:Mol):
    if molecule is None:
        ...
        sp3_carbons = [atom for atom in molecule.GetAtoms() if atom.GetSymbol() and atom.GetHybridization() == HybridizationType.SP3]
...
def calculate_fingerprints(molecules, fingerprint_type="morgan", radius=2, nBits=1024):
    ...
    if fingerprint_type not in valid_fingerprints:
        raise ValueError(f"Invalid fingerprint type. Choose from {', '.join(valid_fingerprints)}.")

    # Create fingerprint generator based on type
    if fingerprint_type == "morgan":
        def fp_generator(mol):  # Pass the molecule as an argument
            return Chem.rdMolDescriptors.GetMorganFingerprintAsBitVect(mol, radius=radius, nBits=nBits)
        ...
        raise ValueError(f"Internal error: Unknown fingerprint type {fingerprint_type}.")
...
def get_representations(dataset, feature_list=None, fingerprint_type = 'morgan', radius=2, nBits=1024, as_array=True):
    features = None
    if isinstance(dataset, pd.DataFrame):  # If dataset is a DataFrame
        if feature_list is None or len(feature_list) == 0:
            ...
            return features
    elif isinstance(dataset, (list, np.ndarray)):  # If dataset is a list of RDKit molecules
        # print("\n\nLIST")
        if all(isinstance(mol, Chem.Mol) for mol in dataset):
            features = calculate_fingerprints(molecules=dataset, fingerprint_type=fingerprint_type, radius=2, nBits=nBits)
            # print("features", features)
            if as_array:
                return np.array(features)
            else:
                return features
        elif all(isinstance(feats, (list, np.ndarray)) and isinstance(feats[0], (int, float, complex)) for feats in dataset):
            # print("\n\nREST")
            return dataset
        else:
