...
class ClusterSplitter(object):
    ...
    def generate_fingerprints(
        ...
        if fingerprint_type == "morgan":
            ...
            fprinter = rdFingerprintGenerator.GetTopologicalTorsionGenerator(
                fpSize=fp_size,
                countSimulation=count_bits,
                includeChirality=include_chirality,
            )
    ...
    def cluster_fingerprints(
        fingerprints, sim_cutoff=0.7, return_as_indices: bool = False
    ):
    ...
    def cluster_molecules(
        ...
        clusters = ClusterSplitter.cluster_fingerprints(
            fingerprints=fingerprints, sim_cutoff=sim_cutoff, return_as_indices=True
        )
        ...
        try:
            ...
            def idx_to_mols(indices, molecules):
                assert np.greater_equal(
                    len(molecules), len(indices)
                ), "The length of arg 2 must be greater or equal that of arg 1."
            ...
            clusters = []
            if isinstance(molecules, (list, np.ndarray, series.Series)) and all(
                [isinstance(molecule, Mol) for molecule in molecules]
            ):
                clusters = ClusterSplitter.cluster_molecules(
                    molecules=molecules,
                    fingerprint_type=fingerprint_type,
            ...
            elif isinstance(molecules, (list, np.ndarray)) and all(
                [isinstance(molecule, list) for molecule in molecules]
            ):
                clusters = ClusterSplitter.cluster_fingerprints(
            ...
            if num_repeats > 1 and not random_cluster_pick:
                warnings.warn(
                    f"random_cluster_pick is set to False while num_repeats > 1({num_repeats}). random_cluster_pick will be set to True."
                )
            ...
            for rcounter in range(num_repeats):
                ...
                train_val = [
                    cluster_mols[k] for k in cluster_picking_order if not k in picked
                ]
                    ...
                    for i in counters:
                        if test_ratio > 0:
                            ...
                            current_folds.append(
                                (
                                    list(
                                        chain(*[splits[k] for k in counters if k != i])
                                    ),
                                    splits[i],
                                    test,
                                )
