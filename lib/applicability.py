...
def get_knn_distances(target, nn_model, k=5, return_mean=True):
    if isinstance(nn_model, (KDTree, BallTree)):
        ...
        raise NotImplementedError("This function is implemented only for KDTree and BallTree models.")


class ApplicationDomainEstimator:
    def __init__(self, nn_model=None, max_dist:float=None, n_neighbors:int=5
                 , metric:str='jaccard', leaf_size:int=40):
    ...
    def from_dict(cls, params_:dict):
        ...
        valid = all([not nn_model is None, not max_dist is None,
                     not n_neighbors is None, not metric is None
                     , not leaf_size is None])
        if valid:
            return cls(
                nn_model=nn_model, max_dist=max_dist,
                n_neighbors=n_neighbors, metric=metric
            )
        else:
            raise ValueError("All of the following arguments must be non-null: 'nn_model', 'max_dist', 'n_neighbors', 'metric'.")

    def to_dict(self):
        return {
            'nn_model':{
                'data': self.nn_model.data.shape
            },
    ...
    def train_nn_model(self, training_data, leaf_size=None):
            ...
            if leaf_size is None:
                ...
                self.leaf_size = leaf_size
            ball_tree_model = BallTree(training_data, leaf_size=leaf_size, metric=self.metric)
            self.nn_model = ball_tree_model        


    def find_max_distance_treshold(self, test_data, step=0.05, perc=90, n_neighbors=5, max_dist=None):
        """
        ...
        """
        assert (not(self.metric =='jaccard' and (not max_dist is None) and max_dist>1)), "The max jaccard distance cannot be greater that 1."
        assert (not (self.n_neighbors is None and n_neighbors is None)), "The default n_neighbors is None. Make sure to provide a non-null value"
        ...
        if self.n_neighbors is None:
            ...
            warnings.warn(f"You provided a n_neighbors params that is differnet from the default value. This will impact the domain of applicability estimation. The default value has been modified to {n_neighbors}.")
            self.n_neighbors = n_neighbors
        ...
        for i in trc:
            nn_k_distances = self.nn_model.query([i], k=self.n_neighbors, return_distance=True)[0]
            avg_k_nn_distances_train.append(nn_k_distances.mean())
        # print(avg_k_nn_distances_train)

        avg_dist = round(sum(avg_k_nn_distances_train)/len(avg_k_nn_distances_train), 3)
        ...
        print(f"Min: {min_dist} - Max = {max_dist} - Avg: {avg_dist} - {perc}th-percentile: {pdval}")

        nn_k_distances_test_to_train = [self.nn_model.query([j], k=self.n_neighbors, return_distance=True)[0].mean() for j in tec]
        ...
        while threshold<max_dist:
            ...
            if self.metric == 'jaccard' and threshold>1.0:
                raise ValueError("The threshold reached passed the max value of 1.0 imposed for the jaccard metric. Consider reducing the value of 'perc', and/or max_dist.")

        valid = [int(x<=threshold) for x in avg_k_nn_distances_train]
        valid_perc = round(sum(valid)/len(valid), 3)
        print(f"Percentage of training data points with avg. distance to {self.n_neighbors}-nearest training data neighbors < {round(threshold, 3)}: {valid_perc}")            

        valid_test_points = [int(y<=threshold) for y in nn_k_distances_test_to_train]
        valid_perc_test   = round(sum(valid_test_points)/len(valid_test_points), 3)
        print(f"Percentage of test data points with avg. distance to {self.n_neighbors}-nearest training data neighbors  < {round(threshold, 3)}: {valid_perc_test}")
