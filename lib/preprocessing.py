...
def select_features(
    df, target_col, model_type="random_forest", threshold=None, **kwargs
):
    """
    ...
    if hasattr(model, "feature_importances_"):
        ...
        raise AttributeError(
            "Model does not have attribute 'feature_importances_' or 'coef_'."
        )
...
def clean_features(
    ...
    numeric_cols = [
        col for col in columns_to_clean if features_df[col].dtype in [int, float]
    ]
    ...
    if len(numeric_cols) > 0:
        ...
        # print(num_cleaned)
        cleaned_features_df[numeric_cols] = pd.DataFrame(
            num_cleaned, columns=numeric_cols
        )

    if not strategy_cat is None:
        categorical_cols = [
            col for col in columns_to_clean if features_df[col].dtype == "object"
        ]
        if len(categorical_cols) > 0:
            imputer_cat = SimpleImputer(missing_values=np.nan, strategy=strategy_cat)
            cat_cleaned = imputer_cat.fit_transform(
                cleaned_features_df[categorical_cols]
            )
            cleaned_features_df[categorical_cols] = pd.DataFrame(
                cat_cleaned, columns=categorical_cols
            )
    ...
    if not standardizer is None:
        ...
        if len(columns_to_scale) > 0:
            cleaned_features_df[columns_to_scale] = standardizer.fit_transform(
...
def clean_data(dd2_data_df, target_column, smiles_column, mol_column="RMol"):
    ...
    print(
        "\nNumber of row with undefined label: ",
        dd2_data_df[target_column].isna().sum(),
    )
    ...
    dd2_data_df = dd2_data_df.reset_index(
        drop=True
    )  ## Reset the index to avoid issues when manipulating based on indices
    ...
    if number_of_duplicates > 0:
        print(
            ...
            duplicates[["largest_frag_ikey", target_column]].head(10),
        )
        print("\n\nRemoving duplicates...\n")
        ...
        if len(indices_to_remove):
            ...
            print(
                dd2_data_df[["largest_frag_ikey", "PCT_INHIB_DD2", target_column]].iloc[
                    indices_to_remove[0]
                ]
            )
        ...
        print(
            "dd2_data_df.shape (after removing conflicting replicates) = ",
            dd2_data_df.shape,
        )
...
def add_custom_features(
    data_df: pd.DataFrame, bool_to_int: bool = True, mol_column="Mol"
):
    data_df["num_halo_atoms"] = data_df[mol_column].apply(featurizers.num_halogen_atoms)
    data_df["halo_weight_ratio"] = data_df[mol_column].apply(
        featurizers.halogen_weight_ratio
    )
    data_df["num_s3_carbons"] = data_df[mol_column].apply(featurizers.num_sp3_carbons)
    data_df["frac_spe_carbons"] = data_df[mol_column].apply(
        featurizers.frac_sp3_carbons
    )
    data_df["lipinski_ro5"] = data_df.apply(
        lambda x: featurizers.matches_lipinski_ro5(
            num_hba=x["CalcNumLipinskiHBA"],
            num_hbd=x["CalcNumLipinskiHBD"],
            molwt=x["CalcExactMolWt"],
            logp=x["MolLogP"],
        ),
        axis=1,
    )
